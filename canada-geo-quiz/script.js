(function () {
  ...
  function buildSession() {
    var prepared = CanadaGeoQuizEngine.prepareQuestions(CanadaGeoQuizQuestions, Date.now());
    state = CanadaGeoQuizEngine.createSession(prepared);
  ...
  function getQuestionHelper(question) {
    if (question.type === "multi") {
      return "Тут кілька правильних відповідей. Збери картину з кількох фактів і лише потім підтвердь вибір.";
    }
    return "Тут лише одна правильна відповідь. Шукай не випадкову деталь, а саму ідею місця або галузі.";
  }
  ...
  function getVisualBlurb(question) {
    if (question.type === "multi") {
      return "Тут треба скласти картину з кількох правильних фактів.";
    }
    return "За правильною відповіддю стоїть конкретний регіон, маршрут або ресурс.";
  }
    ...
    dom.questionCard.innerHTML =
      ...
      '<div class="question-panel">' +
        ...
        '<div class="question-actions">' +
          '<p class="question-helper">Літери й порядок відповідей змінюються щоразу, тож пройти сайт по пам' +
          "яті не вийде.</p>" +
          '<div class="question-actions__buttons">' +
            (question.type === "multi"
              ? '<button class="ghost-button" id="submit-answer" type="button" disabled>Підтвердити</button>'
              : "") +
  ...
  function lockAnswers(question, selectedIds, result, timeoutLabel) {
    ...
    if (timeoutLabel) {
      ...
      revealFact(result.isCorrect ? "Так, це воно" : "Майже, але ні", question);
  ...
  function submitAnswer(selectedIds) {
    ...
    var result = CanadaGeoQuizEngine.gradeAnswer(question, selectedIds);
    state = CanadaGeoQuizEngine.applyAnswer(state, result.isCorrect, timeLeft, question.type);
  ...
  function handleTimeout() {
    ...
    state = CanadaGeoQuizEngine.applyTimeout(state);
  ...
  function renderResult() {
    ...
    dom.resultTitle.textContent = state.resultTitle || CanadaGeoQuizEngine.getResultTitle(state.correctCount);
    dom.resultSummary.textContent =
      "Ти зібрав " + state.correctCount + " правильних відповідей із " + total +
      " і пройшов маршрут від портів і прерій до кленового сиропу, копалин, логістики та промислових міст Канади.";
  ...
  function goNext() {
    state = CanadaGeoQuizEngine.goNext(state);
